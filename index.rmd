---
title: "DispersionCalculator v0.1 Memo"
...
Let $\mathcal{X}$, $\mathcal{Y}$, and $\mathcal{Z}$ be the sets of grid cell indexes. We first esimate the areas under the concentrations curve in the direction of the desired axis using Riemann sums. (This esimate could be improved in the future by using more sophisicated numerical integration techniques). 

$$
C_{ij}(t) = \sum_{k=min(\mathcal{Z})}^{max(\mathcal{Z})} c_{ijk}(t) \Delta z_{ijk}(t)
$$ 
...
* $C_{ij}(t)$ is the esimated integral of $c_{ijk}(t)$ along the Z direction


In R, We do this calculation simultaneously for all timesteps using the following array operations:
...
* `dz` is a 3D array of z-direction dimensions with the same sturcture as `cz`

Next we esimate the first moments of the concentration curves along the desired axis using the formula: 

$$
z^{\prime}_{ij}(t) = \frac{\sum_{k=min(\mathcal{Z})}^{max(\mathcal{Z})} z_{ijk}(t) c_{ijk}(t) \Delta z_{ijk}(t)}{C_{ij}(t)}
$$
...
$$
z^{\prime\prime}_{ij}(t) = \frac{\sum_{k=min(\mathcal{Z})}^{max(\mathcal{Z})} z_{ijk}^2(t) c_{ijk}(t) \Delta z_{ijk}(t)}{C_{ij}(t)} - z^{\prime 2}_{ij}(t)
$$
...
Next, we calculate the concentration-averaged centralized second moments at each timestep over the plane formed by the other two axes.

$$
z^{\prime\prime}_{mean}(t) = \frac{\sum_{i=min(\mathcal{X})}^{max(\mathcal{X})}\sum_{j=min(\mathcal{Y})}^{max(\mathcal{Y})} z^{\prime\prime}_{ij}(t) C_{ij}(t) \overline{\delta_{ij}}(t)}{\sum_{i=min(\mathcal{X})}^{max(\mathcal{X})}\sum_{j=min(\mathcal{Y})}^{max(\mathcal{Y})} C_{ij}(t) \overline{\delta_{ij}}(t)}
$$
